...
single-job lock, callback ordering, the one-script/one-elevation install,
and the "completion always fires" contract.
"""
...
def install_env(monkeypatch):
    """Fake tool detection + elevation so installs never touch the host."""
    state = {"installed": False, "elevated": []}

    monkeypatch.setattr(
        service_mod.detector, "detect",
        lambda tool: (state["installed"], "1.2.3" if state["installed"] else None,
                      "/usr/bin/" + tool.key if state["installed"] else None),
    )
    monkeypatch.setattr(
        service_mod.linux, "detect_package_manager",
        lambda: (service_mod.linux.APT, "APT (Debian/Ubuntu)"),
    )
    monkeypatch.setattr(
        service_mod.linux, "describe_elevation_method",
        lambda: "pkexec (graphical polkit prompt)",
    )

    def fake_elevated(self, cmd, emit):
        state["elevated"].append(cmd)
        emit("$ " + " ".join(cmd))
        state["installed"] = True  # pretend the script worked
        return 0

    monkeypatch.setattr(KubbyService, "_run_elevated_streaming", fake_elevated)
    return state


class TestInstalls:
    def test_install_all_uses_one_script_under_one_elevation(self, install_env):
        lines: list[str] = []
        svc = KubbyService(on_log=lines.append)

        result = svc.install_all()

        assert result["ok"] is True, result
        assert sorted(result["installed"]) == ["helm", "kubectl", "minikube", "podman"]
        assert result["failed"] == []
        assert result["skipped"] == 0

        # Exactly one elevation call, and it's `sh -c <combined script>`.
        assert len(install_env["elevated"]) == 1
        argv = install_env["elevated"][0]
        assert argv[:2] == ("sh", "-c")
        script = argv[2]

        # `set -e` first, then every tool's command in the same script.
        assert script.splitlines()[0] == "set -e"
        assert script.count("# ---") == 4
        for fragment in (
            "minikube-linux-amd64",        # minikube install_script
            "get-helm-4",                  # helm install_script
            "apt-get install -y podman",   # podman via pkg_name + APT
            "dl.k8s.io",                   # kubectl install_script
        ):
            assert fragment in script, fragment

        # One banner, one per-tool success line, one summary.
        assert any("Installing 4 tool(s) via pkexec" in line for line in lines)
        assert sum(1 for line in lines if line.startswith("✓ ")) == 4
        assert any("Done — 4 installed, 0 failed" in line for line in lines)

    def test_install_all_noop_when_everything_is_installed(self, install_env):
        install_env["installed"] = True
        lines: list[str] = []
        svc = KubbyService(on_log=lines.append)
        result = svc.install_all()
        assert result == {
            "ok": True, "installed": [], "failed": [], "skipped": 4,
        }
        assert install_env["elevated"] == []
        assert lines == ["All tools are already installed."]

    def test_install_all_reports_per_tool_post_verify(
        self, install_env, monkeypatch
    ):
        """Script exits 0 but two tools still aren't on PATH → counted failed."""
        # Discovery must see all four missing; post-verify (which runs after
        # the script) sees only helm + podman land.
        phase = {"when": "discover"}

        def flaky_detect(tool):
            if phase["when"] == "installed" and tool.key in ("helm", "podman"):
                return True, "1.0.0", "/usr/bin/" + tool.key
            return False, None, None

        def fake_elevated(self, cmd, emit):
            install_env["elevated"].append(cmd)
            emit("ran")
            phase["when"] = "installed"
            return 0

        monkeypatch.setattr(service_mod.detector, "detect", flaky_detect)
        monkeypatch.setattr(KubbyService, "_run_elevated_streaming", fake_elevated)

        result = KubbyService().install_all()

        assert result["ok"] is False
        assert sorted(result["installed"]) == ["helm", "podman"]
        assert sorted(result["failed"]) == ["kubectl", "minikube"]

    def test_install_tool_refused_while_a_job_runs(self, monkeypatch):
        svc = KubbyService()
        release = start_blocking_job(svc, monkeypatch)
        try:
            result = svc.install_tool("kubectl")
            assert result["ok"] is False
    ...
    def test_install_tool_refused_while_another_install_runs(self, install_env):
        svc = KubbyService()
        svc._install_active = True
        try:
            result = svc.install_tool("kubectl")
            assert result["ok"] is False
            assert "install is already in progress" in result["error"]
        finally:
            svc._install_active = False

    def test_install_all_refused_while_another_install_runs(self, install_env):
        svc = KubbyService()
        svc._install_active = True
        try:
            result = svc.install_all()
    ...
    def test_install_slot_is_released_after_failure(self, install_env, monkeypatch):
        """A failed install must not leave the slot claimed forever."""
        calls: list[tuple] = []

        def fail_elevated(self, cmd, emit):
            calls.append(cmd)
            emit("! nope")
            return 1

        monkeypatch.setattr(KubbyService, "_run_elevated_streaming", fail_elevated)

        svc = KubbyService()
        first = svc.install_all()
        assert first["ok"] is False
        assert svc.is_install_active() is False

        # Slot is genuinely free: a second attempt reaches the elevation step
        # instead of being refused as "already in progress".
        second = svc.install_all()
        assert "already in progress" not in (second.get("error") or "")
        assert len(calls) == 2

    def test_install_tool_unknown_key(self):
        svc = KubbyService()
        result = svc.install_tool("nope")
        assert result["ok"] is False
        assert "Unknown tool" in result["error"]
        assert svc.is_install_active() is False

    def test_install_tool_streams_and_verifies(self, install_env):
        lines: list[str] = []
        svc = KubbyService(on_log=lines.append)
        result = svc.install_tool("podman")

        assert result["ok"] is True
        assert result["version"] == "1.2.3"
        assert result["path"] == "/usr/bin/podman"
        # pkg_name tool → package manager path, not an upstream script.
        assert install_env["elevated"] == [("apt-get", "install", "-y", "podman")]
        assert any(line.startswith("$ apt-get install -y podman") for line in lines)
        assert any(line.startswith("✓ installed at") for line in lines)
